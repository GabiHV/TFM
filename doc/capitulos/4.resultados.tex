...
En este apartado se mostrará de manera fotográfica las funcionalidades desarrolladas en la aplicación y se relacionarán con los 
casos de uso y/o las pruebas que cubren.
...
Con esta funcionalidad pueden constatarse las pruebas relacionadas con la visualización de datos (e.g. CU-04, CU-05 y CU-08) y
la representación de los marcos (e.g. CU-03, CU-06, CU-07, CU-10 y CU-11).
...
Con esta funcionalidad pueden constatarse las pruebas relacionadas con la visualización de marcos (CU-06) puesto que al 
visualizar el \textit{frame} objetivo, se puede observar cómo se desplaza en el mundo virtual y cómo se representa su trayectoria.
...
De igual forma a la velocidad lineal, con esta funcionalidad puede constatarse las pruebas relacionadas con la visualización de marcos (CU-06).

En la Figura \ref{fig:asignacion_frames} se puede observar la funcionalidad de asignación de marcos a las etiquetas \textit{AprilTag}. 
El usuario capturará una etiqueta con la cámara de su dispositivo, seleccionará un \textit{frame} y el tema 
donde se publique la ruta planificada para su visualización y pulsará en el botón de asignación.
Esta funcionalidad permite cubrir el caso de uso de asignación de marcos a etiquetas \textit{AprilTag} (CU-03).
...
Esta funcionalidad cubre el caso de usuario de visualización/ocultación de marcos (CU-06).
...
Esta funcionalidad permite cubrir el caso de uso de teleoperación de robots (CU-07).
...
Esta funcionalidad cubre el caso de uso de planificación de rutas (CU-09).

La Figura \ref{fig:measure_frames} muestra la funcionalidad de medidas entre frames. El usuario seleccionará dos \textit{frames}; estos cambiarán de color a 
azul parpadeante y se mostrará en la ventana izquieda diferentes parámetros como su distancia en metros y su rotación. 
Esto permite cubrir el caso de uso de medición entre marcos (CU-09).
...
Esta funcionalidad cubre el caso de uso de ajuste de posición (CU-10) y escala (CU-11) de marcos.
...
Esta funcionalidad permite cubrir el caso de uso de visualización de nubes de puntos (CU-04).
...
Esto cubre el caso de uso de suscripción a \textit{topics} (CU-12).
...
Esta funcionalidad permite cubrir el caso de uso de publicación en \textit{topics} (CU-13).
...
Esto cubre el caso de uso de modificación de parámetros (CU-14).
...
Esta funcionalidad permite cubrir el caso de uso de configuración de la conexión (CU-01) y del idioma (CU-02).
\begin{figure}[htbp]
    \centering
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{imagenes/manual_usuario/reasignacion.png}
    \caption{Asignación de marcos a etiquetas \textit{AprilTag}}
    \label{fig:asignacion_frames}
\end{figure}
\begin{figure}[htbp]
    \centering
...
RF-015 & Cumplido \\
\hline
\caption{Estado de los requisitos funcionales y no funcionales. Parte 2.}
\end{longtable}
...
RF-017 & Cumplido \\
\hline
RF-018 & Cumplido \footnote{El requisito se entiendo como cumplido aunque existan problemas de rendimiento en la visualización puesto que la funcionalidad se encuentra implementada, es accesible al usuario y puede utilizarse.} \\
\hline
RF-019 & Cumplido \\
\hline
RF-020 & Cumplido \\
...
Como se puede observar, todos los requisitos funcionales han sido cubiertos en el desarrollo de la aplicación. Por otro lado, los no funcionales no 
...
\footnote{Las pruebas concretas pueden comprobarse en \href{https://github.com/GabiHV/TFM/issues?q=is\%3Aissue\%20state\%3Aclosed\%20label\%3Atesting}{GitHub}}.
...
\caption{Resultados de las pruebas de integración. Parte 1.}
\end{longtable}
...
CP-36 & 8 & 8 & 0 \\
\hline
\caption{Resultados de las pruebas de integración. Parte 2.}
\end{longtable}
...
Aun con todos estas técnicas, debido a que el redibujado de las mallas de puntos es costosa a nivel computacional y que puesto a que la aplicación está 
